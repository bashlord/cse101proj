...
tsp_2::tsp_2(int* parentmst, int** input,  int size){
...
void tsp_2::addEdge(int u, int v){
...
int tsp_2::DFSUtil(){
	...
	while(!mystack.empty()){
		...
		cout << "START OF WHILE LOOP size of the stack: " << mystack.size()<<" Node " << node <<endl;
...
/*
	...
	//				cout << node << " has size " << adj[node].size() << " current CHILD OF NODE---- " << *i << endl;
		...
		cout<< "THE MUTHA FUCKIN TSP2 DISTANCE MOFUKA: " << distance << endl;
